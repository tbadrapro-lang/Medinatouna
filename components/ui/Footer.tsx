...
const SOCIALS_BASE = [
  { label: 'Instagram', icon: SOCIAL_ICONS.instagram },
  { label: 'Facebook', icon: SOCIAL_ICONS.facebook },
  { label: 'TikTok', icon: SOCIAL_ICONS.tiktok },
  {
    label: 'WhatsApp',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
export default function Footer() {
  ...
  const SOCIALS = SOCIALS_BASE.map((s) => ({
    ...s,
    href: s.label === 'Instagram' ? CONFIG.INSTAGRAM
      : s.label === 'TikTok' ? CONFIG.TIKTOK
      : s.label === 'WhatsApp' ? waLink(CONFIG.WHATSAPP_FR, "Bonjour, je souhaite obtenir des informations sur vos services (institut de langue, camp bédouin, e-books). Pourriez-vous me contacter ? Merci.")
      : '#',
  }))
  return (
    <footer className="relative bg-night px-6 pt-24 pb-10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 border-t border-white/5 pt-16 pb-10">
          ...
          <div>
            <h3 className="font-display text-lg font-semibold mb-4 text-ivory">Légal</h3>
            <ul className="space-y-2">
              {LEGAL_LINKS.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="font-body text-sm text-ivory/60 hover:text-gold transition-colors">
                    {item.label}
                  </Link>
                </li>
              ))}
