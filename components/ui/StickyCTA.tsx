...
export default function StickyCTA() {
  ...
  const scrollToInstitut = () => {
    document.getElementById('institut')?.scrollIntoView({ behavior: 'smooth' })
  }

  return (
    <div
      className="md:hidden fixed bottom-0 left-0 right-0 z-[60] border-t border-gold/25"
      style={{
        background: 'rgba(4,13,8,.97)',
        backdropFilter: 'blur(24px)',
        paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))',
      }}
    >
      <div className="flex gap-3 p-3 pb-0">
        <button onClick={scrollToInstitut} className="btn-gold flex-1 justify-center">
          Réserver
        </button>
        <a
